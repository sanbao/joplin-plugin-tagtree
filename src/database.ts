...
export async function getNoteSizeByTage(){
    return await runQuery('all', `SELECT nt.tag_id ,COUNT(1) ct from note_tags nt group by nt.tag_id `, {})
}
 ...
 ***************************************************************************************************************************************************/
 export async function setupDatabase(){
    ...
    var databasePath = pluginDir.split('/plugin-data')[0] + "/database.sqlite"
    await fs.ensureDir(pluginDir)
    database = new sqlite3.Database(databasePath)
    return getNoteSizeByTage()
}
